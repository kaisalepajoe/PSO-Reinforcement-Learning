...
def unpack_params(params):
	...
	layers = [(hidden_nodes,224), (2,hidden_nodes)]
	unpacked_params = []
	e = 0
...
def evaluation_function(vector):
	cat_or_flower = np.random.randint(1,3)
	if cat_or_flower == 1:
		directory = 'cat'
		img_number = str(np.random.randint(1,885))
	elif cat_or_flower == 2:
		directory = 'flower'
		img_number = "0"+str(np.random.randint(1,633))
	leading_zeros = 4-len(img_number)
	for zero in range(leading_zeros):
		img_number = "0" + img_number
	path = f"images/natural_images/{directory}/{directory}_{img_number}.jpg"
	img_array = imageio.imread(path)
	img_t = process_image(img_array)
	print(img_t.shape)

	global hidden_nodes
	hidden_nodes = 25
	vector_length = 224*hidden_nodes + 2*hidden_nodes + hidden_nodes + 2
	random_vector = np.random.uniform(-10,10,vector_length)
	params = unpack_params(random_vector)
	classification = model(img_t, params)
	print(classification.shape)

###################################################################

# Training the ANN for image classification

def train(N=9, time_steps=50, repetitions=1,\
	nodes=25, search_space=10, show_animation=True, render=False, disable_progress_bar=False, plot=True):
	'''
